	...
	protected void onReset() {
		...
		//requestService.sendRequest(request, new AsyncCallback<AbstractResponse>() {
			...
			//public void onSuccess(AbstractResponse result) {
				/*if (result.GetResponseType() == ResponseTypeEnum.CATEGORY_ADMINISTRATION_CONTENT_RESPONSE) {
					CategoryAdministrationPageContentResponse response = (CategoryAdministrationPageContentResponse) result;

					List<CategoryCoreContent> categoryCoreContentList = response.getCategoryCoreContentList();
					...
					CellTable<Category> dataGrid = getView().getCategoryList().getDataGrid();
